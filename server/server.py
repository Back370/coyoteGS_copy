...
class Server(RoomManager):
    ...
    def on_disconnect(self,sid):
        ...
        for room in self.rooms.rooms.values():
            ...
            if room_id and player_name:
                break
        if room_id and player_name:
            self.on_leave_room(sid,{"room_id": room_id, "player_name": player_name, "player_sid": sid})
        else:
    ...
    def join_room(self,sid,data):
        ...
        if not is_observer:
            ...
            player_name_list = [(self.rooms.get_room(room_id).players[player_sid]["name"], player_sid) for player_sid in player_sid_list]
            ...
            if room is None:
                ...
                self.sio.emit("room_message", {
                    'header': 'join_room',
                    'msg':f'There is NO ROOM id: {room_id}'
                }, to=sid)
            else:
                player_sid_list = self.rooms.get_room(room_id).players.keys()
                player_name_list = [(self.rooms.get_room(room_id).players[player_sid]["name"], player_sid) for player_sid in player_sid_list]
    ...
    def game_start(self,sid="",data={},):
        ...
        if room:
            ...
            self.sio.emit('room_message',post_data, room=room_id)
            self.ob_emit("room_message",post_data,room_id)
            print(f"Game {current_game_index}/{total_game_num} has started in room {room_id}")
    ...
    def round_start(self,data):
        ...
        if room:
            ...
            for sid, player_name, is_ai in room.active_players:
                ...
                self.sio.emit(
                    "draw_card",{
                        "card": room.players[sid]["card"]
                    },
                    to=sid
                ) 
            for sid, player_name, is_ai in room.active_players:
                room.players[sid]["others_card_sum"] = room.convert_card([player["card"] for other_sid, player in room.players.items() if other_sid != sid], True)#自分以外のすべてのカードの合計値を計算する 
                print(f"sumsum is {room.players[sid]["others_card_sum"]}")

            #ここでplayer["card"]の中身を書き換える(特殊カードの処理)
            room.card_sum = room.convert_card([player["card"] for player in room.players.values()], False)#すべてのカードの合計値を計算する 
            room.start_new_round(round_num)
            self.turn_start({"room_id": room_id})
    
    def turn_start(self,data):
        def get_others_info(player_sid):
            next_sid = (room.active_players.index((player_sid, room.players[player_sid]["name"], room.players[player_sid]["is_ai"])) + 1) % len(room.active_players)
            prev_sid = (room.active_players.index((player_sid, room.players[player_sid]["name"], room.players[player_sid]["is_ai"])) - 1) % len(room.active_players)
            return [
                {
                    "sid": sid, 
            ...
            if turn_player_sid:
                ...
                self.sio.emit(
                        "turn_handling",
                        {
                            ...
                            "sum": room.players[turn_player_sid]["others_card_sum"], #自分以外のすべてのカードの合計値を計算する
                            "log": log_info,
        ...
        if room:
            ...
            if action == -1:  # コヨーテ発動
                ...
                if current_turn_index > 0:
                    prev_player_action = room.logs["round_info"][-1]["turn_info"][current_turn_index-1]["action"]
                    print(f"全体の合計値は{room.card_sum}です") 
                    if(prev_player_action > room.card_sum):#すべての場のカードの合計を計算する
                ...
                if is_coyote_success:
                    ...
                    prev_turn_index = (room.current_turn_index - 1) % len(room.active_players)
                ...
                self.round_end({"room_id": room_id, "is_coyote_success": is_coyote_success})
            else:
        ...
        if room:
            ...
            for(sid, player_data) in room.players.items():
                if(sid == active_sid for active_sid in room.active_players):
                    room.deck.cashed_cards.append(player_data["card"]) # 引いたカードを捨て札に追加
    ...
    def result(self,data):
        ...
        if room:
            ...
            death_ranking = [{"player_name": player_data["name"]} for sid, player_data,is_ai in room.death_players]
            ...
            if is_game_continue:
                ...
                self.game_start(data={"room_id": room_id, "game_num": room.current_game_index})
    ...
    def send_room_info(self,sid,data):
        ...
        if room:
            ...
            "game_num": room.game_num,
            "round_num": room.round_num,
            "players": [{"sid": sid, "name": player["name"], "life": player["life"]} for sid, player in room.players.items()],
            "active_players": [{"sid": sid, "name": name, "is_ai": is_ai} for sid, name, is_ai in room.active_players]
            }
    ...
    def send_rooms(self,sid):
        ...
        for room_id, room in self.rooms.rooms.items():
            rooms_info.append({
                ...
                "game_num": room.game_num,
                "round_num": room.round_num,
                "players": [{"name": player["name"], "life": player["life"]} for player in room.players.values()],
                "active_players": [{"name": name} for _, name in room.active_players]
            })
    ...
    def start(self):
        ...
        eventlet.wsgi.server(eventlet.listen(('', self.port)), self.app, log_output=False)   # type: ignore
